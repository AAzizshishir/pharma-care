...
const addReview = async (customerId, payload) => {
    ...
    const medicineInOrder = order.orderItems.find((item) => item.medicineId === payload.medicineId);
    if (!medicineInOrder) {
...
const getCustomerReview = async (customerId) => {
...
const getSellerMedicineReviews = async (sellerId) => {
    const result = await prisma.review.findMany({
