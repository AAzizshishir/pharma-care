...
const createOrder = async ({ customerId, shippingAddress, items, }) => {
    let totalAmount = 0;
...
const getUserOrderById = async (customerId, orderId) => {
...
// update order status
const updateOrderStatus = async (orderId, sellerId, payload) => {
...
const cancellOrder = async (orderId, payload) => {
    const result = await prisma.order.update({
        where: {
            id: orderId,
        },
